  ...
  if (loading) {
    return (
      <div className="flex flex-col gap-6 py-6 items-center justify-center min-h-[400px]">
        ...
        <span className="text-text-muted font-bold text-sm">Загрузка азбуки Катакана...</span>
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-surface/30 border border-border/10 p-5 rounded-3xl backdrop-blur-md">
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            <h1 className="text-3xl font-extrabold text-text">Катакана</h1>
            <Badge variant="info" className="font-bold">カタカナ</Badge>
          </div>
          <p className="text-text-secondary text-sm font-medium mt-1">
            Слоговая азбука японского языка, используемая преимущественно для записи заимствованных слов, имен собственных и ономатопеи.
          </p>
        </div>
            ...
            <div className="flex justify-between items-start">
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-text-muted mt-0.5">Азбука: Катакана</span>
                </div>
              </div>
              <button
            ...
            {selectedChar.examples && selectedChar.examples.length > 0 && (
              <div className="flex flex-col gap-2.5 mt-2 bg-bg-secondary/40 p-3.5 rounded-2xl border border-border/5">
                <span className="text-xs font-bold text-text-secondary flex items-center gap-1">
                  <BookOpen className="w-3.5 h-3.5" /> Примеры слов (заимствования)
                </span>
